...
func (s *Server) HandleListOpsforUserID(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.Svc.ListOpsforUserID(r.Context(), userID)
	if err != nil {
		err = apperr.WithAttrs(
			fmt.Errorf("could not get operations for user: %w", err),
